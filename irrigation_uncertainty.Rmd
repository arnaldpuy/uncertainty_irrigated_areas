---
title: "The future environmental impact of irrigated agriculture is grossly underestimated"
...
loadPackages(c("data.table", "fitdistrplus", "fGarch", "readxl", "countrycode",
               ...
               "doParallel", "sensitivity", "wesanderson", "sensobol", 
...
AB <- lapply(Continents, function(Continents) sobol_matrices(n = n, 
                                                             k = k) %>%
               data.table()) 
...
model <- function(X) {
  ...
  # COMPUTE THE MODEL ---------------------------
  Y <- X[, Y0] + 
    10 ^ (Intercept + Slope * Beta + X[, epsilon]) * Beta * X[, r] * 
    (N ^ (1 - X[, gamma]) + X[, r] * X[, t] * (1 - X[, gamma])) ^
    ((Beta + X[, gamma] - 1) / (1 - X[, gamma])) 
...
```{r plot_uncertainty_final, cache=TRUE, dependson=c("global_uncertainty", "quantiles", "arrange_output"), dev="tikz", fig.height=3.8, fig.width=6.5}
...
a <- AB.dt %>%
  ...
  geom_rect(data = cropland.1,
            ...
            inherit.aes = FALSE) +
  geom_histogram() +
  scale_fill_discrete(name = "Cropland in 2050",
                      labels = c("Gross", "Net")) +
...
```{r global_only, cache=TRUE, dependson="plot_uncertainty", dev="tikz", fig.height=2.5, fig.width=3.3}

b +
  theme(plot.margin = margin(l = 0, unit = "cm"))
```
...
```{r plot_scatterplots, cache=TRUE, dependson="sensitivity_scatterplots", fig.height=8.4, fig.width=5.4}
...
```{r plot_sobol, cache=TRUE, dependson="prepare_plot_sobol", dev="tikz", fig.height=5, fig.width=6.2}
