---
title: "Current models severely underestimate future irrigated areas"
author: "Arnald Puy, Samuele Lo Piano and Andrea Saltelli"
header-includes:
...
link-citations: yes
fontsize: 11pt
bibliography: /Users/arnald/Documents/bibtex/LATEX_population.bib
---
...
## Irrigated area versus population size

```{r datasets, warning=FALSE, cache=TRUE}
...
## Irrigated area versus population density

```{r density_population, cache=TRUE, dependson="datasets", dev="tikz", fig.height=8.7, fig.width=4, fig.cap="Scatter plots of measures of population density against irrigated areas."}
...
## Plot irrigated area versus population size / population density

```{r population_vs_irrigation, dependson="datasets", fig.align="center", fig.height=4.7, dev="tikz", figh.width=5.5, cache=TRUE, fig.cap="Scatter plots of irrigated areas against population. The strip label indicates the data set used to plot the values for irrigated areas [@FAO2016b; @FAO2017a; @Meier2018; @Salmon2015; @Siebert2013; @Thenkabail2009]. All those data sets have been compiled and studied by @Meier2018. The population data was retrieved from the @UnitedNations2017c."}
...
## Irrigated area versus water withdrawal / water requirement

```{r read_aquastat_dataset, cache=TRUE}
...
```{r compute_water_requirements, cache=TRUE, dependson=c("read_aquastat_dataset", "function_code")}
...
## Plot irrigated area versus water withdrawal / water requirement

```{r plot_water_variables, cache=TRUE, dependson="compute_water_requirements", dev="tikz", fig.height=8.7, fig.width=4}
...
```{r plot_country_level, dev="tikz", cache=TRUE, message=FALSE, warning=FALSE}
...
```{r plot_country_level1, dev="tikz", cache=TRUE, fig.height=9, fig.width=6, message=FALSE, warning=FALSE, fig.cap="Extension of irrigation at the country level. The data was retrieved from @Meier2018."}
...
```{r plot_country_level2, dev="tikz", cache=TRUE, fig.height=9, fig.width=6, message=FALSE, warning=FALSE, fig.cap="Extension of irrigation at the country level. The data was retrieved from @Meier2018."}
...
```{r out, fig.keep="none", cache=TRUE, dependson="datasets"}
...
```{r outlier_export, dependson="out", cache=TRUE}
...
```{r out_plot, dev="tikz", dependson="out", fig.width=10, fig.height=9, cache=TRUE, fig.cap="Scatter plot of Mahalanobis vs Robust distances."}
...
```{r plot_out_population, cache=TRUE, dev="tikz", dependson="out_plot", fig.width=10, fig.height=9, cache=TRUE, fig.cap="Scatter plot of Mahalanobis vs Robust distances."}
...
```{r plot_out_water, cache=TRUE, dev="tikz", dependson="out_plot", fig.width=10, fig.height=9, cache=TRUE, fig.cap="Scatter plot of Mahalanobis vs Robust distances."}
...
## Irrigated area baseline values ($Y_0$)}

```{r irrigated_area_baseline, dependson="datasets", cache=TRUE}
...
## Growth rate between population and irrigated area ($\beta$), irrigated area and water required for irrigation ($\phi$, $\delta$), and noise ($\varepsilon$)

```{r define_bootstrap_replicas, cache=TRUE}
...
```{r ols_regressions, dependson=c("datasets", "compute_water_requirements", "define_bootstrap_replicas"), cache=TRUE}
...
```{r ols_regressions_robust, dependson=c("datasets", "compute_water_requirements", "define_bootstrap_replicas"), cache=TRUE}
...
```{r sma_regressions, dependson=c("datasets", "compute_water_requirements", "define_bootstrap_replicas"), cache=TRUE}
...
```{r extract_alpha_beta_robust, cache=TRUE, dependson="sma_regression"}
...
```{r export_sma, cache=TRUE, dependson="extract_alpha_beta_robust"}

fwrite(sma.nonrobust.pop, "sma.nonrobust.pop.csv")
fwrite(sma.robust.pop, "sma.robust.pop.csv")
```

```{r dt_bootstrap_samples, cache=TRUE, dependson=c("sma_regressions", "ols_regressions_robust", "ols_regressions", "extract_alpha_beta_robust")}
...
```{r export_bootstrap_samples2, cache=TRUE, dependson="dt_bootstrap_samples"}
...
```{r predict_epsilon, cache=TRUE, dependson="dt_bootstrap_samples"}
...
```{r lookup_tables, cache=TRUE, dependson="dt_bootstrap_samples"}
...
```{r export_bootstrap_samples, cache=TRUE, dependson=c("dt_bootstrap_samples", "lookup_tables")}
...
## Population baseline values ($N$)

```{r population_baseline, cache=TRUE, dependson="datasets"}
...
```{r population_growth, cache=TRUE, dependson="datasets", dev="tikz", fig.height=6, fig.width=5}
...
## Cropland available ($K$)

```{r cropland_available, cache=TRUE, dependson="datasets"}
...
```{r settings_sample_matrix, cache=TRUE, results="hide", message=FALSE}
...
```{r sample_matrix_cluster, cache=TRUE, dependson="settings_sample_matrix", results="hide", message=FALSE}
...
```{r number_boot_replicas, cache=TRUE, dependson="dt_bootstrap_samples"}
...
```{r transform_sample_matrix, cache=TRUE, dependson=c("sample_matrix_cluster", "water_available", "cropland_available", "population_growth", "population_baseline", "lookup_tables", "settings_sample_matrix")}
...
```{r final.dt, cache=TRUE, dependson=c("sample_matrix_cluster", "water_available", "cropland_available", "population_growth", "population_baseline", "lookup_tables", "settings_sample_matrix", "transform_sample_matrix")}
...
```{r export.final.dt, cache=TRUE, dependson=c("sample_matrix_cluster", "water_available", "cropland_available", "population_growth", "population_baseline", "transform_sample_matrix", "lookup_tables", "settings_sample_matrix", "transform_outliers")}
...
```{r run_model, cache=TRUE, dependson=c("model", "export.final.dt")}
...
```{r arrange_output, cache.lazy = FALSE, cache=TRUE, dependson=c("run_model", "final.dt", "transform_outliers", "export.final.dt")}
...
```{r export_full_model output, cache=TRUE, dependson="arrange_output"}
...
```{r export_AB_matrix, cache=TRUE, dependson="arrange_output"}
...
```{r global_uncertainty, cache=TRUE, dependson="arrange_output"}
...
```{r plot_uncertainty_final, cache=TRUE, dependson=c("global_uncertainty", "quantiles", "arrange_output", "read_projections"), dev="tikz", fig.height=4.5, fig.width=6.5}
...
```{r extra_plot, cache=TRUE, dependson=c("global_uncertainty", "quantiles", "arrange_output", "read_projections", "plot_uncertainty_final"), dev="tikz", fig.height=6.8, fig.width=4.7}
...
```{r global_only, cache=TRUE, dependson=c("plot_uncertainty", "extra_plot"), dev="tikz", fig.height=2.5, fig.width=4.7}

b + theme(plot.margin = margin(l = 0, unit = "cm"))
```


```{r export_global, cache=TRUE, dependson="global_uncertainty"}
...
```{r uncertainty_statistics, cache=TRUE, dependson=c("arrange_output", "global_uncertainty")}
...
```{r sensitivity_scatterplots, cache=TRUE, dependson="arrange_output"}
...
```{r plot_scatterplots, cache=TRUE, dependson="sensitivity_scatterplots", fig.height=8.5, fig.width=4.7}
...
```{r sobol_functions, cache=TRUE, echo=FALSE}
...
```{r sobol_settings, cache=TRUE, dependson="arrange_output"}
...
```{r sobol_indices, cache=TRUE, dependson=c("arrange_output", "sobol_settings", "sobol_functions")}
...
```{r sobol_ci, cache=TRUE, dependson=c("sobol_indices", "sobol_settings", "sobol_functions")}
...
```{r sobol_indices_dummy, cache=TRUE, dependson=c("arrange_output", "sobol_settings", "sobol_functions")}
...
```{r export_sobol_indices, cache=TRUE, dependson=c("sobol_ci_dummy", "sobol_ci", "sobol_settings", "sobol_functions")}
...
```{r prepare_plot_sobol, cache=TRUE, dependson=c("sobol_ci_dummy", "sobol_ci", "sobol_settings", "sobol_functions")}
...
```{r plot_sobol, cache=TRUE, dependson="prepare_plot_sobol", dev="tikz", fig.height=5.3, fig.width=6.2}
...
```{r prepare_plot_sobol2, cache=TRUE, dependson=c("sobol_ci_dummy", "sobol_ci", "sobol_settings", "sobol_functions"), dev="tikz", fig.height=5, fig.width=5.2}
...
```{r check_sum_si, cache=TRUE, dependson="export_sobol_indices"} 
...
# Appendix: relation between projections of irrigated areas and population

```{r pop_model, cache=TRUE}
...
```{r run_pop_model, cache=TRUE, dependson=c("arrange_output", "pop_model")}
...
```{r arrange_pop_data, cache=TRUE, dependson=c("preliminary steps", "arrange_output", "global_uncertainty")}
...
```{r plot_pop_plots, cache=TRUE, dependson="arrange_pop_data"}
...
```{r plot_pop_plots_final, cache=TRUE, dependson="plot_pop_plots", fig.height=8.5, fig.width=6}
